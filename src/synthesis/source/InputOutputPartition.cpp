...
InputOutputPartition InputOutputPartition::read_from_file(
  ...
  // input_substr = CosySyft::split(line, ":");
  //
  // if (input_substr.size() != 2 || input_substr[0] != ".inputs") {
  //   throw bad_file_format_exception(line_number);
  // }
  //
  // std::string trimmed_input_substr = CosySyft::trim(input_substr[1]); // remove leading and trailing whitespace
  // partition.input_variables = CosySyft::split(trimmed_input_substr, " ");
  //
  // ++line_number;
  // std::getline(in, line);
  //
  // std::vector<std::string> output_substr;
  // output_substr = CosySyft::split(line, ":");
  //
  // if (output_substr.size() != 2 || output_substr[0] != ".outputs") {
  //   throw bad_file_format_exception(line_number);
  // }
  //
  // std::string trimmed_output_substr = CosySyft::trim(output_substr[1]); // remove leading and trailing whitespace
  // partition.output_variables = CosySyft::split(trimmed_output_substr, " ");
